 ...
 *
 * Ported from
 * https://cdcvs.fnal.gov/redmine/projects/dune-raw-data/repository/revisions/develop/entry/dune-raw-data/Overlays/CTB_content.h
 *
 * Developer: willpvazquez (27 July 2022)
 *
 *  Originally Created on: Oct 8, 2017
 *      Author: nbarros
 */

#ifndef CTBMODULES_SRC_CTBPACKETCONTENT_HPP_
#define CTBMODULES_SRC_CTBPACKETCONTENT_HPP_
...
typedef struct ch_status_t
{
  ...
  bool get_state_crt(const uint16_t channel) { return ((crt & (0x1 << channel)) != 0x0); }

  bool get_state_pds(const uint16_t channel) { return ((pds & (0x1 << channel)) != 0x0); }

  bool get_state_beam(const uint16_t channel) { return (((beam_hi << 4 | beam_lo) & (0x1 << channel)) != 0x0); }
} ch_status_t;
...
typedef struct trigger_t
{
  ...
  {
    if (IsHLT())
      return trigger_word & (0x1 << i);
    if (IsLLT())
      return i == 0 ? false : trigger_word & (0x1 << (i - 1));
    return false;
  }
} trigger_t;
...
#endif /* CTBMODULES_SRC_CTBPACKETCONTENT_HPP_  */
