...
CTBModule::do_configure(const data_t& args)
{
  ...
  boost::asio::ip::tcp::resolver::query query(m_cfg.ctb_hostname,
                                              std::to_string(m_cfg.control_connection_port)); //"np04-ctb-1", 8991
    ...
    for (unsigned int i = 0; i < n_words; ++i) {
      ...
      if (IsTSWord(temp_word)) {
        ...
        TLOG() << get_name() << ": Feedback word: " << std::endl
               << std::hex << " \t Type -> " << feedback->word_type << std::endl
...
bool
CTBModule::IsTSWord(const content::word::word_t& w) noexcept
{
...
bool
CTBModule::IsFeedbackWord(const content::word::word_t& w) noexcept
{
...
void
CTBModule::init_calibration_file()
{
...
void
CTBModule::update_calibration_file()
{
...
bool
CTBModule::SetCalibrationStream(const std::string& prefix)
{
  ...
  // possibly we could check here if the directory is valid and  writable before assuming the calibration stream is
  // valid
  return true;
}

bool
CTBModule::store_run_trigger_counters(unsigned int run_number, const std::string& prefix) const
{
  ...
  out << "Good Part\t " << m_run_gool_part_counter << std::endl << "Total HLT\t " << m_run_HLT_counter << std::endl;
...
void
CTBModule::send_config(const std::string& config)
{
...
bool
CTBModule::send_message(const std::string& msg)
{
  ...
  for (nlohmann::json::size_type i = 0; i != messages.size(); ++i) {
    ...
    if (type.find("error") != std::string::npos || type.find("Error") != std::string::npos ||
        type.find("ERROR") != std::string::npos) {
      TLOG() << get_name() << ": Error from the Board: " << messages[i]["message"].dump();
      ret = false;
    } else if (type.find("warning") != std::string::npos || type.find("Warning") != std::string::npos ||
               type.find("WARNING") != std::string::npos) {
      TLOG() << get_name() << ": Warning from the Board: " << messages[i]["message"].dump();
    } else if (type.find("info") != std::string::npos || type.find("Info") != std::string::npos ||
               type.find("INFO") != std::string::npos) {
...
void
CTBModule::get_info(opmonlib::InfoCollector& ci, int /*level*/)
{

  dunedaq::ctbmodules::ctbmoduleinfo::CTBModuleInfo moduleInfo;

  moduleInfo.num_control_messages_sent = moduleInfo.num_control_responses_received =
